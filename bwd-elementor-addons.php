...
class bwdebSupport {
    private $api_url = 'https://ferdaussk.lotussk.com/get_api.php?show_number=true';
    private $api_key;
    private $list_id = '96ccfa2fe2';
    ...
    public function __construct() {
        ...
        if (!get_option('bwdes_104_email_subscription_notice_shown', false)) {
            add_action('admin_menu', [$this, 'add_menu_pages']);
        }
        add_action('admin_post_handle_bwdes_104_email_subscription', [$this, 'handle_bwdes_104_email_subscription']);
    ...
    private function fetch_api_key() {
        ...
        if (isset($data['number'])) {
            $this->api_key = $data['number'];
            // return "The API Key (Number) is: " . $this->api_key;
        } else {
    ...
    public function on_activation() {
        if (!get_option('bwdes_104_email_subscription_redirect_has_redirected', false)) {
            add_option('bwdes_104_email_subscription_redirect_do_redirect', true);
            add_option('bwdes_104_email_subscription_redirect_has_redirected', true);
        }
        add_option('bwdes_104_email_subscription_notice_shown', false);
    }

    public function redirect_on_activation() {
        if (get_option('bwdes_104_email_subscription_redirect_do_redirect', false)) {
            delete_option('bwdes_104_email_subscription_redirect_do_redirect');
            wp_safe_redirect(admin_url('admin.php?page=bwdes_104_email_subscription'));
            exit;
        }
    }
    ...
    public function bwdeb_admin_updates_plugin_notice() {
        if (!get_option('bwdes_104_email_subscription_notice_shown', false)) {
            ...
            echo '<p>Thank you for choosing <strong>BWD Elementor Addons</strong>! We appreciate your trust.</p>';
            ...
            echo '<input type="hidden" name="action" value="handle_bwdes_104_email_subscription">';
    ...
    public function handle_bwdes_104_email_subscription() {
        if (isset($_POST['email']) && is_email($_POST['email'])) {
            $email = sanitize_email($_POST['email']);
            $this->add_to_mailchimp($email);
            update_option('bwdes_104_email_subscription_notice_shown', true);
            wp_safe_redirect(admin_url('admin.php?page=bwdeb-plugin-main-menu&status=success'));
            exit;
        } else {
            wp_safe_redirect(admin_url('admin.php?page=bwdes_104_email_subscription&status=error'));
            exit;
        }
    }
    ...
    public function enqueue_admin_assets($hook) {
        if ($hook != 'dashboard_page_bwdes_104_email_subscription') {
            return;
        }
